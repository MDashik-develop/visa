      ...
      <ul class="space-y-2 font-medium">
         @if (auth()->check() && auth()->user()->role === 'admin')
               ...
               <li>
                  <button type="button"
                     class="flex items-center w-full p-2 text-base transition duration-75 rounded-lg group 
                     {{ Request::is('admin/message*') ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-600 bg-gray-200 hover:text-white' }}"
                     aria-controls="dropdown-message" data-collapse-toggle="dropdown-message">
                     <svg class="shrink-0 w-5 h-5 text-black group-hover:text-white
                           {{ Request::is('admin/massage*') ? ' text-white' : ' hover:text-white' }}"
                           aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                           viewBox="0 0 18 21">
                           <path
                              d="M15 12a1 1 0 0 0 .962-.726l2-7A1 1 0 0 0 17 3H3.77L3.175.745A1 1 0 0 0 2.208 0H1a1 1 0 0 0 0 2h.438l.6 2.255v.019l2 7 .746 2.986A3 3 0 1 0 9 17a2.966 2.966 0 0 0-.184-1h2.368c-.118.32-.18.659-.184 1a3 3 0 1 0 3-3H6.78l-.5-2H15Z" />
                     </svg>
                     <span class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">Massages's</span>
                     <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none"
                           viewBox="0 0 10 6">
                           <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                              stroke-width="2" d="m1 1 4 4 4-4" />
                  ...
                  <ul id="dropdown-message"
                     class="{{ Request::is('message*') ? '' : 'hidden' }} py-2 space-y-2">
                     <li>
                           <a href="{{ route('admin.message') }}"
                              class="flex items-center w-full p-2 pl-11 transition duration-75 rounded-lg group 
                              {{ Request::routeIs('admin.message') ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-600 hover:text-white' }}">
                              All Message
                           </a>
                     </li>
                     <li>
                           <a href="{{ route('admin.subscribe') }}"
                              class="flex items-center w-full p-2 pl-11 transition duration-75 rounded-lg group 
                              {{ Request::routeIs('admin.subscribe') ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-600 hover:text-white' }}">
                              All Subscribe
                           </a>
                     </li>
                  </ul>
         ...
         <li>
               <button type="button"
                  class="flex items-center w-full p-2 text-base transition duration-75 rounded-lg group 
                  {{ Request::is('/profile*') ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-600 bg-gray-200 hover:text-white' }}"
                  aria-controls="dropdown-profile" data-collapse-toggle="dropdown-profile">
                  <svg class="shrink-0 w-5 h-5 text-black group-hover:text-white
                     {{ Request::is('/profile*') ? ' text-white' : ' hover:text-white' }}"
                     aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                     viewBox="0 0 18 21">
                     <path
                           d="M15 12a1 1 0 0 0 .962-.726l2-7A1 1 0 0 0 17 3H3.77L3.175.745A1 1 0 0 0 2.208 0H1a1 1 0 0 0 0 2h.438l.6 2.255v.019l2 7 .746 2.986A3 3 0 1 0 9 17a2.966 2.966 0 0 0-.184-1h2.368c-.118.32-.18.659-.184 1a3 3 0 1 0 3-3H6.78l-.5-2H15Z" />
               ...
               <ul id="dropdown-profile"
                  class="{{ Request::is('profile*') ? '' : 'hidden' }} py-2 space-y-2">
                  <li>
                     <a href="{{ route('profile.edit') }}"
                           class="flex items-center w-full p-2 pl-11 transition duration-75 rounded-lg group 
                           {{ Request::routeIs('profile.edit') ? 'bg-indigo-600 text-white' : 'hover:bg-indigo-600 hover:text-white' }}">
                           My Profile
                     </a>
                  </li>
               </ul>
